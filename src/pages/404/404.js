...
function mapCharAt(x, y) {
    if (x > 0) {
        ...
        x = mapWidth - ((Math.ceil(x) * -1) % mapWidth) - 1;
    }

    if (y > 0) {
        y = Math.floor(y) % mapHeight;
    } else {
        y = mapHeight - ((Math.ceil(y) * -1) % mapHeight) - 1;
    }
...
class The404Page {
    ...
    constructor(canvas) {
        ...
        plugins["Router"].mainClass("fullscreen");
...
*/
    ...
    drawPlayer() {
        ...
        // TODO: Results still okay when map has scrolled ???
...
ko.components.register("the-404-page", {
    viewModel: {
        createViewModel(params, componentInfo) {
            let canvas = componentInfo.element.getElementsByTagName("canvas")[0];
            return new The404Page(canvas);
        }
    },
